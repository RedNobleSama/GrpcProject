...
type GoodsServer struct {
	in.UnimplementedGoodsServer
}

func ModelToResponse(goods model.Goods) in.GoodsInfoResponse {
	return in.GoodsInfoResponse{
		Id:              goods.ID,
		...
		Images:          goods.Images,
		Category: &in.CategoryBriefInfoResponse{
...
//func (s *GoodsServer) GoodsList(ctx context.Context, req *in.GoodsFilterRequest) (*in.GoodsListResponse, error) {
...
func (s *GoodsServer) GoodsList(ctx context.Context, req *in.GoodsFilterRequest) (*in.GoodsListResponse, error) {
	//关键词搜索、查询新品、查询热门商品、通过价格区间筛选、通过商品分类筛选
	var goodsListResponse in.GoodsListResponse
	var goods []model.Goods
	//queryMap := map[string]rpc{}{}
	localDb := db.DB.Model(model.Goods{})
	if req.KeyWords != "" {
		//搜索
	...
	if req.TopCategory > 0 {
		var category model.Category
		if result := db.DB.First(&category, req.TopCategory); result.RowsAffected == 0 {
			return nil, status.Errorf(codes.NotFound, "商品分类不存在")
		}
...
func (s *GoodsServer) BatchGetGoods(ctx context.Context, req *in.BatchGoodsIdInfo) (*in.GoodsListResponse, error) {
	goodsListResponse := &in.GoodsListResponse{}
	var goods []model.Goods

	//调用where并不会真正执行sql 只是用来生成sql的 当调用find， first才会去执行sql，
	result := db.DB.Where(req.Id).Find(&goods)
	for _, good := range goods {
		goodsInfoResponse := ModelToResponse(good)
		goodsListResponse.Data = append(goodsListResponse.Data, &goodsInfoResponse)
	}
...
func (s *GoodsServer) GetGoodsDetail(ctx context.Context, req *in.GoodInfoRequest) (*in.GoodsInfoResponse, error) {
	var goods model.Goods

	if result := db.DB.Preload("Category").Preload("Brands").First(&goods, req.Id); result.RowsAffected == 0 {
...
func (s *GoodsServer) CreateGoods(ctx context.Context, req *in.CreateGoodsInfo) (*in.GoodsInfoResponse, error) {
	var category model.Category
...
func (s *GoodsServer) DeleteGoods(ctx context.Context, req *in.DeleteGoodsInfo) (*emptypb.Empty, error) {
	if result := db.DB.Delete(&model.Goods{BaseModel: model.BaseModel{ID: req.Id}}, req.Id); result.Error != nil {
		return nil, status.Errorf(codes.NotFound, "商品不存在")
	}
	return &emptypb.Empty{}, nil
}

func (s *GoodsServer) UpdateGoods(ctx context.Context, req *in.CreateGoodsInfo) (*emptypb.Empty, error) {
	var goods model.Goods

	if result := db.DB.First(&goods, req.Id); result.RowsAffected == 0 {
		return nil, status.Errorf(codes.NotFound, "商品不存在")
	}
